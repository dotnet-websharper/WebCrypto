...
function Main(){
  ...
  const t=(this_1.h.push(new VarStr("password", passwordInput())),this_1);
  const this_2=(t.h.push(EventQ2(t.k, "hashpassword", () => t.i, () => {
    const _3=null;
    StartImmediate(Delay(() => Bind(AsAsync(hashPassword()), () => Return(null))), null);
  })),t);
  const b=(this_2.h.push(new TextView("hashedpassword", hashedPassword().View)),this_2);
  const p=CompleteHoles(b.k, b.h, [["password", 0, null]]);
...
function hashedPassword(){
  return _c.hashedPassword;
}
function passwordInput(){
  return _c.passwordInput;
}
function hashPassword(){
  ...
  return new Promise((_2) => {
    ...
    let _3=crypto().subtle.digest("SHA-256", data).then((d) => {
      ...
      const hexString=concat_1("", map((byte_1) => padNumLeft(byte_1.toString(16), 2), init(hashArray_1.length, (i) => hashArray_1[i])));
      hashedPassword().Set("Hashed: "+String(hexString));
