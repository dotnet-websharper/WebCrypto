...
function e(){ const fY=null;return new Promise((fZ) => { const f0=d().Get();const f1=eval("new TextEncoder()").encode(f0);let f2=f().subtle.digest("SHA-256", f1).then((f3) => { const f4=new Uint8Array(f3);(((f6) => f6(String(f4)))((f6) => { console.log(f6);}));const f5=bh("", u0033((f6) => bg(f6.toString(16), 2), u0032(f4.length, (f6) => f4[f6])));c().Set("Hashed: "+String(f5));return Promise.resolve(null);});fZ(f2);});}
function f(){ return L.crypto;}
...
let L=fS((fY) => class fZ { static { L=fY(this);} static crypto;static hashedPassword;static passwordInput;static { this.passwordInput=bL.Create_1("");this.hashedPassword=bL.Create_1("Waiting for input...");this.crypto=globalThis.crypto;} });
